...
public class OrderStatusViewHandler {
    ...
    public void whenOrderPlaced_then_CREATE_1 (@Payload OrderPlaced orderPlaced) {
        try {
            ...
            orderStatus.setAddress(String.valueOf(orderPlaced.getAddress()));
            orderStatus.setCustomerId(orderPlaced.getCustomerId());
            orderStatus.setQty(orderPlaced.getQty());
            orderStatus.setStoreId(String.valueOf(orderPlaced.getStoreId()));
    ...
    public void whenPaymentApproved_then_UPDATE_4(@Payload PaymentApproved paymentApproved) {
        try {
            if (!paymentApproved.validate()) return;
                ...
                for(OrderStatus orderStatus : orderStatusList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    orderStatus.setPaymentId(String.valueOf(paymentApproved.getId()));
